...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando civel/ acima
- hook_state_load/save    → gerencia civel/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/civel/cowork-state.json."""
    start = start.resolve()
