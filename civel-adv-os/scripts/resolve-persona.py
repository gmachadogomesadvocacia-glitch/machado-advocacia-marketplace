...
1. Env var CIV_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.CIV_PERSONA
3. ~/.config/civel-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "civel-adv-os" / "active-cowork.json",
        home / ".civel-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[civel-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-civel)\n")
    else:
        sys.stderr.write(f"[civel-adv-os] Persona carregada de {source}: {path}\n")
