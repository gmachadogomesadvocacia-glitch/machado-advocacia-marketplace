...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando consumidor/ acima
- hook_state_load/save    → gerencia consumidor/.hook-state.json (debounce anti-flap)
...
STATE_DIR = "consumidor"
HOOK_STATE_FILENAME = ".hook-state.json"


def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/consumidor/cowork-state.json."""
    start = start.resolve()
