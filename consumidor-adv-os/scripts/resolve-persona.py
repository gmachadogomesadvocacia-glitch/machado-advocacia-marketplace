...
1. Env var CONSUM_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.CONSUM_PERSONA
3. ~/.config/consumidor-adv-os/active-cowork.json contem persona_path
...
def _try_env_var() -> Path | None:
    p = os.environ.get("CONSUM_PERSONA")
    if p:
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "consumidor-adv-os" / "active-cowork.json",
        home / ".consumidor-adv-os" / "active-cowork.json",
    ):
...
def resolve_persona() -> tuple[Path, str]:
    ...
    for fn, tag in (
        (_try_env_var, "env:CONSUM_PERSONA"),
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[consumidor-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-consumidor)\n")
    else:
        sys.stderr.write(f"[consumidor-adv-os] Persona carregada de {source}: {path}\n")
