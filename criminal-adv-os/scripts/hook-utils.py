...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando criminal/ acima
- hook_state_load/save    → gerencia criminal/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/criminal/cowork-state.json."""
    start = start.resolve()
