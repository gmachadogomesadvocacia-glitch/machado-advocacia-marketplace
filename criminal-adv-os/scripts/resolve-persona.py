...
1. Env var CRIM_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.CRIM_PERSONA
3. ~/.config/criminal-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "criminal-adv-os" / "active-cowork.json",
        home / ".criminal-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[criminal-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-criminal)\n")
    else:
        sys.stderr.write(f"[criminal-adv-os] Persona carregada de {source}: {path}\n")
