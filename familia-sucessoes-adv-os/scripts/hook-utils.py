...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando familia/ acima
- hook_state_load/save    → gerencia familia/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/familia/cowork-state.json."""
    start = start.resolve()
