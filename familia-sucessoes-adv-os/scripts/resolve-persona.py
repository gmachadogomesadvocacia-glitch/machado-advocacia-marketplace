...
1. Env var FAM_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.FAM_PERSONA
3. ~/.config/familia-sucessoes-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "familia-sucessoes-adv-os" / "active-cowork.json",
        home / ".familia-sucessoes-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[familia-sucessoes-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-familia)\n")
    else:
        sys.stderr.write(f"[familia-sucessoes-adv-os] Persona carregada de {source}: {path}\n")
