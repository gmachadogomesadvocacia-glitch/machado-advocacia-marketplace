# CLAUDE.md — manutencao do plugin imobiliario-adv-os
...
├── .gitignore                   # ignora imobiliario/, casos/, CASO.md, arquivos/
├── README.md
...
- **Limite de 11264 bytes por `SKILL.md`.** Skills maiores nao carregam. Ao
  crescer uma skill, fatiar conteudo para `context/` ou arquivos auxiliares.
...
- **Tokens de persona literais em disco.** A persona renderizada
  (`<cwd>/imobiliario/persona.md`) guarda os valores ja resolvidos; as skills
  resolvem placeholders `{{...}}` a partir dela (inclusive `{{FRENTES}}`,
  `{{POLOS}}` e `{{POLO_CLIENTE}}`). Nao hardcodar identidade de escritorio nas skills.
- **Nao versionar casos.** `imobiliario/`, `casos/`, `CASO.md`, `arquivos/` ficam
  no `.gitignore` (sigilo OAB + LGPD). Nunca remover essas entradas.

## Armadilha central do dominio

- **Posse x propriedade x dominio (PA-05) e a armadilha central.** Toda skill e
  todo command que toca posse/titulo/registro deve separar os tres planos: posse
  (fato), propriedade (direito real), dominio/registro (matricula). A escolha da
  via (possessoria x petitoria) e da peca depende disso. Nunca deduzir dominio na
  possessoria (art. 557 CPC). Manter essa separacao ao editar qualquer conteudo.

## Convencoes de entrega

- **Nomenclatura de arquivos novos:** `AAAA-MM-DD - Cliente - tipo.ext`
  (ex.: `2026-06-16 - Fulano - acao de despejo.txt`).
...
2. Se for Tier 2 (producao), encerrar sempre roteando para `revisao-final-imobiliaria`.
...
1. Criar `commands/<nome>.md` no gabarito padrao: frontmatter (`description`,
   `allowed-tools: Read, Write, Edit, Bash, Glob, Grep`, `argument-hint`) + corpo
   curto em portugues com `## PROTOCOLO` e a linha final **Skill a acionar:** `<skill>`.
2. Um command e fino: valida pre-condicoes, aciona UMA skill principal e referencia
   as Proibicoes Absolutas por codigo (PA-04, PA-05, PA-06, ...). A logica pesada
...
- **`scripts/state.py`** — maquina de estados do `cowork-state.json`.
  CLI: `python scripts/state.py init <cowork>` e `... set`.
- **`scripts/resolve-persona.py`** / **`hook-utils.py`** — resolucao da persona
  e utilitarios de hook.
- **STATE_DIR:** `imobiliario/`. **Variaveis de ambiente do engine:**
  `IMOB_PERSONA` (caminho da persona.md), `IMOB_COWORK_PATH` (raiz do workspace),
  `IMOB_STATE_FILE` (cowork-state.json).
