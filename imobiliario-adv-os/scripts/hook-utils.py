...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando imobiliario/ acima
- hook_state_load/save    → gerencia imobiliario/.hook-state.json (debounce anti-flap)
...
STATE_DIR = "imobiliario"
HOOK_STATE_FILENAME = ".hook-state.json"


def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/imobiliario/cowork-state.json."""
    start = start.resolve()
