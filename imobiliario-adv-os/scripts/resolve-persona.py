...
1. Env var IMOB_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.IMOB_PERSONA
3. ~/.config/imobiliario-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "imobiliario-adv-os" / "active-cowork.json",
        home / ".imobiliario-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[imobiliario-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-imobiliario)\n")
    else:
        sys.stderr.write(f"[imobiliario-adv-os] Persona carregada de {source}: {path}\n")
