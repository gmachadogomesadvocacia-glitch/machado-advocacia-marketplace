...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando isencao-ir/ acima
- hook_state_load/save    → gerencia isencao-ir/.hook-state.json (debounce anti-flap)
...
STATE_DIR = "isencao-ir"
HOOK_STATE_FILENAME = ".hook-state.json"


def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/isencao-ir/cowork-state.json."""
    start = start.resolve()
