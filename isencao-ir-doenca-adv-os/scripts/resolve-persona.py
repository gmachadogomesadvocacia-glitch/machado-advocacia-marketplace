...
1. Env var ISIR_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.ISIR_PERSONA
3. ~/.config/isencao-ir-doenca-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "isencao-ir-doenca-adv-os" / "active-cowork.json",
        home / ".isencao-ir-doenca-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[isencao-ir-doenca-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-isencao-ir)\n")
    else:
        sys.stderr.write(f"[isencao-ir-doenca-adv-os] Persona carregada de {source}: {path}\n")
