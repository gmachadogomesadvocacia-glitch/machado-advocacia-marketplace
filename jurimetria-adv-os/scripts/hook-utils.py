...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando jurimetria/ acima
- hook_state_load/save    → gerencia jurimetria/.hook-state.json (debounce anti-flap)
...
STATE_DIR = "jurimetria"
HOOK_STATE_FILENAME = ".hook-state.json"


def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/jurimetria/cowork-state.json."""
    start = start.resolve()
