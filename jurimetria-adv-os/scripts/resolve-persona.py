...
1. Env var JURI_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.JURI_PERSONA
3. ~/.config/jurimetria-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "jurimetria-adv-os" / "active-cowork.json",
        home / ".jurimetria-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[jurimetria-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-jurimetria)\n")
    else:
        sys.stderr.write(f"[jurimetria-adv-os] Persona carregada de {source}: {path}\n")
