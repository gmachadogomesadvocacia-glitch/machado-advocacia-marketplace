...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando leiloes/ acima
- hook_state_load/save    → gerencia leiloes/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/leiloes/cowork-state.json."""
    start = start.resolve()
