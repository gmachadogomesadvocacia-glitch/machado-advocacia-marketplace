...
1. Env var LEIL_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.LEIL_PERSONA
3. ~/.config/leiloes-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "leiloes-adv-os" / "active-cowork.json",
        home / ".leiloes-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[leiloes-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-leiloes)\n")
    else:
        sys.stderr.write(f"[leiloes-adv-os] Persona carregada de {source}: {path}\n")
