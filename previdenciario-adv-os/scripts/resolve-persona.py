...
1. Env var PREV_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.PREV_PERSONA
3. ~/.config/previdenciario-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "previdenciario-adv-os" / "active-cowork.json",
        home / ".previdenciario-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[previdenciario-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-previdenciario)\n")
    else:
        sys.stderr.write(f"[previdenciario-adv-os] Persona carregada de {source}: {path}\n")
