...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando recuperacao-judicial/ acima
- hook_state_load/save    → gerencia recuperacao-judicial/.hook-state.json (debounce anti-flap)
...
STATE_DIR = "recuperacao-judicial"
HOOK_STATE_FILENAME = ".hook-state.json"


def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/recuperacao-judicial/cowork-state.json."""
    start = start.resolve()
