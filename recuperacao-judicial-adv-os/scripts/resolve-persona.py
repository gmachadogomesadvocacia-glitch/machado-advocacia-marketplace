...
1. Env var RJ_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.RJ_PERSONA
3. ~/.config/recuperacao-judicial-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "recuperacao-judicial-adv-os" / "active-cowork.json",
        home / ".recuperacao-judicial-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[recuperacao-judicial-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-rj)\n")
    else:
        sys.stderr.write(f"[recuperacao-judicial-adv-os] Persona carregada de {source}: {path}\n")
