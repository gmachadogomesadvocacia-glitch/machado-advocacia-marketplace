...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando transito/ acima
- hook_state_load/save    → gerencia transito/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/transito/cowork-state.json."""
    start = start.resolve()
