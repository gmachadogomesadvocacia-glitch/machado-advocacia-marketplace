...
1. Env var TRAN_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.TRAN_PERSONA
3. ~/.config/transito-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "transito-adv-os" / "active-cowork.json",
        home / ".transito-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[transito-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-transito)\n")
    else:
        sys.stderr.write(f"[transito-adv-os] Persona carregada de {source}: {path}\n")
