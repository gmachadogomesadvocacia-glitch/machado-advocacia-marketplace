...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando tributario/ acima
- hook_state_load/save    → gerencia tributario/.hook-state.json (debounce anti-flap)
...
STATE_DIR = "tributario"
HOOK_STATE_FILENAME = ".hook-state.json"


def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/tributario/cowork-state.json."""
    start = start.resolve()
