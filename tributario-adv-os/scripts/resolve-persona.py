...
1. Env var TRIB_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.TRIB_PERSONA
3. ~/.config/tributario-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "tributario-adv-os" / "active-cowork.json",
        home / ".tributario-adv-os" / "active-cowork.json",
    ):
