...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando usucapiao/ acima
- hook_state_load/save    → gerencia usucapiao/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/usucapiao/cowork-state.json."""
    start = start.resolve()
