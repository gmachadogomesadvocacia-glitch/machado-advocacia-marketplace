...
1. Env var USU_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.USU_PERSONA
3. ~/.config/usucapiao-adv-os/active-cowork.json contem persona_path
...
def _try_active_cowork_config() -> Path | None:
    home = Path.home()
    for config_path in (
        home / ".config" / "usucapiao-adv-os" / "active-cowork.json",
        home / ".usucapiao-adv-os" / "active-cowork.json",
    ):
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[usucapiao-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start-usucapiao)\n")
    else:
        sys.stderr.write(f"[usucapiao-adv-os] Persona carregada de {source}: {path}\n")
